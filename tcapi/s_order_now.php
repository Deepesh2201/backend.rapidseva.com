...
if ($data['uid'] == '') {
	...
	$table = "tbl_order";
	$field_values = array("uid", "odate", "p_method_id", "address", "o_total", "subtotal", "trans_id", "cid", "add_on", "add_total", "add_per_price", "time", "date", "longs", "lats", "wal_amt", "coupon_id", "coupon_amt", "rid", "htype");
	$data_values = array("$uid", "$timestamp", "$p_method_id", "$full_address", "$product_total", "$product_subtotal", "$transaction_id", "$cid", "$add_on", "$add_total", "$add_per_price", "$time", "$date", "$longs", "$lats", "$wal_amt", "$coupon_id", "$coupon_amt", "$pid", "$htype");
	...
	for ($i = 0; $i < count($ProductData); $i++) {
		...
		if($coupon_amt > 0){
			...
			$table = "tbl_coupon_usage";
			$field_values = array("coupon_id", "user_id", "discount_amt");
			$data_values = array("$coupon_id", "$uid", "$coupon_amt");
